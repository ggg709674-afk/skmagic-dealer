...
1. **크롤**: `python crawl_one.py <gid>` — 색상 변형마다 전부 (detail.html/meta.json/images)
   - 26.5월부터 본사가 움직이는 상세컷을 gif 대신 **`<video src=*.mp4>`로 제공** — crawl_details가 video도 수집함(2026-06-12 수정). 크롤 후 `images/detail_*.mp4` 받아졌는지 확인
   - 구형 제품처럼 **gif로 주는 컷**이면 tools/README.md 절차(다운로드 → ffmpeg mp4 변환 → gif 삭제)
...
### 상세 동영상 컷 누락 — 본사 제공 방식이 바뀌어 있었음
- 형이 발견: mini 상세의 움직이는 컷이 전부 정지 상태. 구형 제품은 본사가 gif로 줘서 우리가 mp4 변환(WORKLOG 11)했는데, **mini(26.5월 컨텐츠)부터 본사가 아예 `<video src=*.mp4>`로 제공** → img만 긁는 크롤러가 동영상 22개를 통째로 놓침
- fix 3종: crawl_details.py가 `img, video` 둘 다 수집(문서 순서 유지) / app.js detailImagesHtml에 `.mp4` 직접 참조 → `<video>` 렌더 분기 추가 / **process_one에 force 재크롤 시 options·specs·specs_by_size 보존 로직 추가**(이거 없으면 재크롤마다 crawl_options/crawl_specs 결과가 날아감)
- mini 재크롤 → 색상당 mp4 22개(21.5MB)·상세 75컷, 사양·옵션 보존 확인
