...
3. **(2026-05-31 5) ★유력 원인 발견 — `init()` 이중 호출**: admin.js 맨 끝이 `document.addEventListener('DOMContentLoaded', init); init();` 라 init 이 **두 번** 돌고 있었음. 스크립트가 body 끝(defer 없음)이라 즉시 1회 + DOMContentLoaded 1회 = 두 init 흐름이 같은 DOM/전역 state 를 두고 경쟁 → 새로고침 때 렌더 출렁임(메뉴 튐) 유발 가능. → `document.readyState==='loading'` 가드로 **정확히 1회만** 실행하도록 수정. + 원인 확정용 진단로그 2줄 추가(`[admin][nav] init start` / `[admin][nav] apply`, hash·menu·kind·readyState 출력).
   - ⚠️ **형이 배포본 admin 에서 #carddiscount 로 새로고침 후 콘솔(F12) 캡처** → 튐 재현되면 `[admin][nav]` 로그(apply 가 menu=products 로 뜨는지/몇 번 도는지) 붙여주면 잔여 원인 특정. 안 튀면 이중 init 이 원인이었던 것 → 진단로그 제거 예정.
